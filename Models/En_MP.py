...
class En_MP(nn.Module):
    def __init__(self,cfg,mode,backbone,n_pretrain_classes,n_finetune_classes):
        ...
        self.LIST_EFFECTIVE_DINO_BLOCKS = np.array([0,6,11])
        ...
        self.temporal_slot = nn.ModuleList([SlotAttentionEncoder(num_iterations=cfg.MYMODEL.TEMPORAL_SLOT.NUM_ITER,
                                            ...
                                            for i in range(len(cfg.MYMODEL.NUM_FRAMES_LIST)*len(self.LIST_EFFECTIVE_DINO_BLOCKS))])                                 
        ...
        self.flow_combination_x_slots = nn.ModuleList([nn.Sequential(
            Block(dim = self.dim, num_heads=8),
            Block(dim = self.dim, num_heads=8),
        )
        for i in range(len(self.LIST_EFFECTIVE_DINO_BLOCKS))])
           
        self.flow_combination_x_time = nn.ModuleList([nn.Sequential(
            Block(dim = self.dim, num_heads=8),
            Block(dim = self.dim, num_heads=8),
        )
        for i in range(len(self.LIST_EFFECTIVE_DINO_BLOCKS))])
        
        self.invar_proj = nn.ModuleList([nn.Sequential(
           nn.Linear(14*14,self.dim, bias=True),
        )  
        for i in range(len(self.LIST_EFFECTIVE_DINO_BLOCKS))])
        
        self.invar_combination_x_axes = nn.ModuleList([nn.Sequential(
            Block(dim = self.dim, num_heads=8),
            Block(dim = self.dim, num_heads=8),
        )
        for i in range(len(self.LIST_EFFECTIVE_DINO_BLOCKS))])
             
        self.invar_combination_x_time = nn.ModuleList([nn.Sequential(
            Block(dim = self.dim, num_heads=8),
            Block(dim = self.dim, num_heads=8),
        )
        for i in range(len(self.LIST_EFFECTIVE_DINO_BLOCKS))])

        self.flow_head = nn.ModuleList([nn.Sequential(
            nn.Linear(self.dim, n_finetune_classes, bias=True)
            )
        for i in range(len(self.LIST_EFFECTIVE_DINO_BLOCKS))])

        self.motion_head = nn.ModuleList([nn.Sequential(
            nn.Linear(self.dim, n_finetune_classes, bias=True)
            )
        for i in range(len(self.LIST_EFFECTIVE_DINO_BLOCKS))])
        
        self.blks_feature_combination = nn.Sequential(
            Block(dim = self.dim, num_heads=8),
            Block(dim = self.dim, num_heads=8),
        )
        
        self.all_head = nn.Linear(self.dim, n_finetune_classes, bias=True)
    ...
    def motion_invar(self,tokens,tau,fs):
        B,T,N,E =  tokens.shape 
        H_t = W_t = int(math.sqrt(N))
        normalized_tokens = F.normalize(tokens, p=2, dim=-1) 
    ...
    def forward(self, frames, labels):
        ...
        for frame_idx in range(T):
            tokens_list,_,_ = self.vit_encoder.get_intermediate_feat(frames[:,:,frame_idx],n=12) 
            tokens = torch.stack(tokens_list,dim=0)[:,:,1:].detach() 
            if frame_idx==0:
                tokens_all=tokens.unsqueeze(2)
            else:
                tokens_all= torch.cat((tokens_all,tokens.unsqueeze(2)),dim=2)
        _,_,_,N,_ =  tokens_all.shape 
        H_t = W_t = int(math.sqrt(N))

        for blk_id in range(len(self.LIST_EFFECTIVE_DINO_BLOCKS)):
            attn_tokens = tokens_all[self.LIST_EFFECTIVE_DINO_BLOCKS[blk_id]]
            for i in range(len(self.NUM_FRAMES_LIST)):
                ...
                # Temporal Slots
                ##########################################################################
                aux_temporal_loss, temporal_slots_all=self.temporal_walk_loss(attn_tokens_OF.view(B,T*H_t*W_t,-1),blk_id*len(self.NUM_FRAMES_LIST)+i) 
                aux_loss +=  1.0*aux_temporal_loss
                
                ##########################################################################
                # Multi-scale Features Concatenation
                ##########################################################################
                normalized_temporal_slots = F.normalize(temporal_slots_all, p=2, dim=-1) 
                ...
                flow_features = self.flow_proj[blk_id*len(self.NUM_FRAMES_LIST)+i](flow_features)  
            ...
            fused_flow_features = self.flow_combination_x_time[blk_id](fused_flow_features_x_slots).mean(dim=1) 
            
            flow_logit = self.flow_head[blk_id](fused_flow_features.view(B,-1))
            ...
            fused_motion_invar_features = self.invar_combination_x_time[blk_id](fused_motion_invar_features_x_axes.view(B,T,-1)).mean(dim=1) 
            
            invar_logit = self.motion_head[blk_id](fused_motion_invar_features.view(B,-1))
            ...
            if blk_id == 0:
                total_features = blk_features
            else:
                total_features = torch.cat((total_features,blk_features),dim=1) 

        all_features = self.blks_feature_combination(total_features).mean(dim=1)
